...
function FluidSimulation({
	...
	useEffect(() => {
		...
		// Define interface for pointer object
		interface Pointer {
			id: number;
			texcoordX: number;
			texcoordY: number;
		...
		// Constructor function for pointer objects
		...
		function getWebGLContext(canvas: HTMLCanvasElement) {
			...
			if (isWebGL2) {
				...
				formatRGBA = getSupportedFormat(gl2, gl2.RGBA16F, gl2.RGBA, halfFloatTexType || gl2.FLOAT);
				formatRG = getSupportedFormat(gl2, gl2.RG16F, gl2.RG, halfFloatTexType || gl2.FLOAT);
				formatR = getSupportedFormat(gl2, gl2.R16F, gl2.RED, halfFloatTexType || gl2.FLOAT);
			} else {
				const gl1 = gl as WebGLRenderingContext;
				formatRGBA = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || gl1.FLOAT);
				formatRG = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || gl1.FLOAT);
				formatR = getSupportedFormat(gl1, gl1.RGBA, gl1.RGBA, halfFloatTexType || gl1.FLOAT);
			}
    ...
    `,
		...
		function resizeCanvas() {
			const width = scaleByPixelRatio((canvas as unknown as HTMLCanvasElement).clientWidth);
			const height = scaleByPixelRatio((canvas as unknown as HTMLCanvasElement).clientHeight);
			if (
				(canvas as unknown as HTMLCanvasElement).width !== width ||
				(canvas as unknown as HTMLCanvasElement).height !== height
			) {
				(canvas as unknown as HTMLCanvasElement).width = width;
				(canvas as unknown as HTMLCanvasElement).height = height;
				return true;
